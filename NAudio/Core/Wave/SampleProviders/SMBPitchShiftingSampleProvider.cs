...
namespace NAudio.Wave.SampleProviders
{
    ...
    public class SmbPitchShiftingSampleProvider : ISampleProvider
    {
        ...
        static readonly float M_PI_2 = MathF.PI / 2f;
        ...
        public int Read(float[] buffer, int offset, int count)
        {
            ...
            {
                var mono = ArrayPool<float>.Shared.Rent(sampRead);
                try
                {
                    var index = 0;
            ...
            {
                var halfSampRead = sampRead >> 1;
                var left = ArrayPool<float>.Shared.Rent(halfSampRead);
                var right = ArrayPool<float>.Shared.Rent(halfSampRead);
                try
                {
                    var index = 0;
                    ...
                    shifterLeft.PitchShift(pitch, halfSampRead, fftSize, osamp, waveFormat.SampleRate, left);
                    shifterRight.PitchShift(pitch, halfSampRead, fftSize, osamp, waveFormat.SampleRate, right);
                    index = 0;
                    for (var sample = offset; sample <= sampRead + offset - 1; sample += 2)
                    {
            ...
            throw new NotSupportedException("Shifting of more than 2 channels is currently not supported.");
        }
        ...
        private float Limiter(float sample)
        {
            ...
            if ((LIM_THRESH < sample))
            {
                ...
                res = MathF.Atan(res) / M_PI_2 * LIM_RANGE + LIM_THRESH;
            }
