...
namespace NAudio.Wave.SampleProviders
{
    class Stereo24SampleChunkConverter : ISampleChunkConverter
    {
        ...
        public bool GetNextSample(out float sampleLeft, out float sampleRight)
        {
            ...
            {
                sampleLeft = (((sbyte)sourceBuffer[offset + 2] << 16) | (sourceBuffer[offset + 1] << 8) | sourceBuffer[offset]) * (1.0f / (1 << 23));
                offset += 3;
                sampleRight = (((sbyte)sourceBuffer[offset + 2] << 16) | (sourceBuffer[offset + 1] << 8) | sourceBuffer[offset]) * (1.0f / (1 << 23));
                offset += 3;
                return true;
            }
            else
